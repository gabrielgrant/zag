...
import { useSignal, useTask$, useVisibleTask$, useOnDocument, $, type QRL } from "@builder.io/qwik"
...
export function useMachine<T extends MachineSchema>(
  ...
  const scope = createScope({
    id: (userProps as any).id,
    ids: (userProps as any).ids,
    getRootNode: (userProps as any).getRootNode,
  })
  ...
  const prop = <K extends keyof typeof props>(key: K): (typeof props)[K] => {
    return props[key]
  }
