...
export default component$(() => {
  const id = useId()
  const activeFile = useSignal<FileItem | null>(null)

  const service = useMachine(menu.machine, () => ({
    id,
    onTriggerValueChange({ value }: menu.TriggerValueChangeDetails) {
    ...
    <>
      <main style={{ padding: "40px", fontFamily: "system-ui" }}>
        ...
        <div style={{ marginTop: "20px", padding: "12px", backgroundColor: "#f9fafb", borderRadius: "6px" }}>
          ...
          {activeFile.value ? `${activeFile.value.icon} ${activeFile.value.name} (${activeFile.value.type})` : "-"}
        </div>
        ...
        <div {...api.getPositionerProps()}>
          ...
          >
            ...
            >
              Open {activeFile.value?.type === "folder" ? "Folder" : "File"}
            </li>
            <li
            ...
            <li style={{ height: "1px", backgroundColor: "#e5e7eb", margin: "4px 0" }} />
            <li
