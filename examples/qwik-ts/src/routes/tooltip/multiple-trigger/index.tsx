...
export default component$(() => {
  const id = useId()
  const activeProduct = useSignal<Product | null>(null)

  const service = useMachine(tooltip.machine, () => ({
    id,
    positioning: { placement: "right" as const },
    onTriggerValueChange({ value }: tooltip.TriggerValueChangeDetails) {
      ...
      <main style={{ padding: "40px", fontFamily: "system-ui" }}>
        <section style={{ marginBottom: "40px" }}>
              ...
              {products.map((product) => (
                <tr key={product.id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                  ...
                  <td style={{ padding: "12px" }}>
                    <button {...api.getTriggerProps({ value: `${product.id}` })}>ⓘ</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div style={{ marginTop: "20px", padding: "12px", backgroundColor: "#f9fafb", borderRadius: "6px" }}>
            ...
            {activeProduct.value ? `${activeProduct.value.name} ($${activeProduct.value.price})` : "-"}
          </div>
        </section>
        ...
        <div {...api.getPositionerProps()}>
          <div {...api.getContentProps()}>
            {activeProduct.value ? (
              <div>
                <strong>{activeProduct.value.name}</strong>
                <p>{activeProduct.value.description}</p>
              </div>
            ) : (
